...
/**
 *
 * Ajax update for the EJSApp view.php to see connected Users on EJSApp lab
 *
 * @package    mod_ejsapp
...
if ($collab_session->master_user == $USER->id){
    $acceptances = $DB->get_records('ejsapp_collab_acceptances', array('collaborative_session' => $colsession));


    $context = context_course::instance($collab_session->course);

    $baseurl = new moodle_url('mod/ejsapp/connected_users.php', array(
        'colsession' => $colsession));

    $tablecolumns = array('fullname');
    $tableheaders = array(get_string('connected_users', 'ejsapp'));

    $table = new flexible_table('user-index-connected' . $collab_session->course);
    ...
    $table->set_attribute('id', 'connected_users');
    ...
    if (count($acceptances) > 0){
        ...
        if (! $valid_user){
            $noUsers = get_string('no_connected_users', 'ejsapp');
